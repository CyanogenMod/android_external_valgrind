...
// Tests missalinged access via PC+$#imm
#define TESTINSTPCMISSALING(instruction, RD, label, cvin) \
{ \
    unsigned int out; \
    unsigned int cpsr; \
    __asm__ volatile(\
...
#define TESTINSTPCMISSALING_DWORDOUT(instruction, label, cvin) \
{ \
    unsigned int out; \
    unsigned int out2; \
    unsigned int cpsr; \
...
#define TESTINSTPCMISSALING_2OUT(instruction, RD, RD2, label, cvin) \
{ \
    unsigned int out; \
    unsigned int out2; \
    unsigned int cpsr; \
...
static int old_main(void)
{
    ...
    TESTINSTPCMISSALING("adr.w r1, label_magic_adrw;and r1, r1, #0x3", r1, label_magic_adrw, cv);
    // omitting LDC/LDC2
    TESTINSTPCMISSALING("ldr r1, label_magic_ldr", r1, label_magic_ldr, cv);
    TESTINSTPCMISSALING("ldrb r1, label_magic_ldrb", r1, label_magic_ldrb, cv);
    TESTINSTPCMISSALING_2OUT("ldrd r0, r1, label_magic_ldrd", r0, r1, label_magic_ldrd, cv);
    TESTINSTPCMISSALING("ldrh r1, label_magic_ldrh", r1, label_magic_ldrh, cv);
    TESTINSTPCMISSALING("ldrsb r1, label_magic_ldrsb", r1, label_magic_ldrsb, cv);
